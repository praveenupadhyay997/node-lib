...
class Server {
  constructor (port, option = {}) {
    ...
    app.use(ratelimit(ratelimitOpt)) // call the static method to get the middleware

    if (corsOpt) {
      // Pass url + domain lists; Cors also whitelists refresh-token headers for session retry after expiry.
      app.use(cors({ url: corsOpt.url, domain: corsOpt.domain }))
    }
    if (swaggerOpt) {
